...
*/ 
       ...
       Delay_Time =  distance_1 / 10 + 1;            // 거리에 비례하는 주기(= Delay_Time * 50 msec )를 갖는 경고음 발생
       ...
       if( Warning_Flag == 1 )
	   {
	       ...
	       {
               PORTB ^= 0x10  ;    // PB4(버저) toggle : 버저 단속음 (XOR = 값이 같으면 0이 되고 같이 다르면 1이 됨)
			   PORTB ^= 0x20  ;    // PB5(LED) toggle :  LED ON, OFF 반복 (X0R)
	       }
	   }
