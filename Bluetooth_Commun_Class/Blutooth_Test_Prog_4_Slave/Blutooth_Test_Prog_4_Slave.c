...
//  Blutooth Command( Master --> Slave )
static volatile char Cmd_Message_1[] = { "Read USonic Sensor1" } ;   
static volatile char Cmd_Message_2[] = { "Read USonic Sensor2" } ;  
...
//	                LcdPuts("Dist_3 =    cm"); 
               ...
               if     ( eq_count1 == 19 )  cmd_data = 1 ;     // 명령 1
               else if( eq_count2 == 19 )  cmd_data = 2 ;     // 명령 2   
         ...
         ////////  명령(Command) 처리 ( Master --> Slave ) : Master가 Slave에게 보내온 명령 처리


			if( cmd_data == 1 )     // 명령 1 이면
			{

		        HexToDec( distance_1,10);                // 초음파센서 1에 의해 측정된 거리 distance_1를 십진수로 변환

                SerialPutString( "Distance 1 = " );      // Master에게 메시지 전송
                ...
                SerialPutChar('.');                      // Master에게 데이터(문자열) 전송시 마침표(.)를 항상 끝에 전송해야함 
//              SerialPutChar('\n');                      // 휴대폰(Master)에게 데이터(문자열) 전송시 마침표(.)를 항상 끝에 전송해야함 
			}

			else if( cmd_data == 2 )     // 명령 2 이면
			{

		        HexToDec( distance_2,10);                // 초음파센서 2에 의해 측정된 거리 distance_2를 십진수로 변환

                SerialPutString( "Distance 2 = " );      // Master에게 메시지 전송
